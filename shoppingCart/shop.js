...
function remove_selection(x) {
    // Prevent going into negative quantities
    if (quantities[x] > 0) {
        quantities[x] = quantities[x] - 1;
        totals[x] = prices[x] * quantities[x];
        totalOrderAmt -= prices[x];
...
function checkout() {
    ...
    // Display the total order amount and the number of items
    document.getElementById("total").innerHTML = "Total Items: " + totalItems + " | Total Amount: R" + totalOrderAmt.toFixed(2);
}
